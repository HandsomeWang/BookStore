...
<div class="divBody">
  <div class="divContent">
    <%--上一页 --%>
<c:choose>
	<c:when test="${pb.pc eq 1 }"><span>上一页</span></c:when>
	<c:otherwise><a href="${pb.url }pc=${pb.pc-1}" class="pager">上一页</a></c:otherwise>
</c:choose>
...
4. 如果end>tp, 让begin=tp-5, end=tp
 ...
 <c:forEach begin="${begin }" end="${end }" var="i">
   <c:choose>
   	  ...
   	  <c:otherwise>
   	    <a href="${pb.url }pc=${i}" class="pager">${i }</a>
   	  </c:otherwise>
   </c:choose>
    ...
    <c:if test="${end < pb.tp }">
     ...
     <%--下一页 --%>
<c:choose>
	<c:when test="${pb.pc eq pb.tp }"><span>下一页</span></c:when>
	<c:otherwise><a href="${pb.url }pc=${pb.pc+1}" class="pager">下一页</a></c:otherwise>
</c:choose>
